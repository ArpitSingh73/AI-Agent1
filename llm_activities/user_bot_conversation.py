...
def extract_date_time(chat_history: list, date_and_time: str):
    ...
    try:
        print(
            f"Generating response for history: {chat_history}"
        )
        ...
        model = genai.GenerativeModel(
           ...
           Based on the data you need to extract the date and time user wants to schedule a meeting or if date and time not given then ask the user about date and time.\
            ...
            If date and time can be extracted then return answer in JSON format with three keys - date, start_time and end_time in utc format same as present date and time, check sample here: {date_and_time}\
            Else current context is not sufficient the return answer in JSON format with one key - insufficient_context with value as - "need more context".\
        ...
        print("Extarcting the initial date and time: ")
